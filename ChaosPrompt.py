...
# ---------------------------
# Time-related words pool
# ---------------------------
TIME_WORDS = [
    "for eternity", "through ages", "beyond time", "in perpetuity", "for eons",
    "across centuries", "through forever", "in timeless void", "beyond moments",
    "for an instant", "through epochs", "in endless night", "across millennia",
    "for always", "through infinity", "in eternal dusk", "beyond dawn",
    "for countless days", "through temporal drift", "in frozen time",
    "across forgotten years", "for one heartbeat", "through ancient hours",
    "in twilight forever", "beyond yesterday", "for tomorrow's echo",
    "through midnight", "in daybreak eternal", "across time's edge"
]
...
def make_prompt(person_mode=False, add_mj_params=True, pure_chaos_mode=False, add_time=False):
    ...
    if person_mode:
        prompt_body = (
            f"A {person_phrase}{time_phrase} standing among {body_words[0]}, {body_words[1]}, {body_words[2]}, "
            f"with hints of {hint_words[0]}, {hint_words[1]}, {hint_words[2]}, "
            f"{random.choice(VISUAL_DESCRIPTORS)}"
        )
    else:
        prompt_body = (
            f"A {body_words[0]} {body_words[1]}{time_phrase} emerging from {body_words[2]}, "
...
with col1:
    ...
    add_time = st.checkbox("⏳ Add temporal element", value=False)
    pure_chaos = st.checkbox("🌀 Pure Chaos Mode (generate invented words)", value=False)
    if st.button("Generate Prompt"):
        st.session_state.prompt = make_prompt(person_mode, add_mj_params=add_mj, pure_chaos_mode=pure_chaos, add_time=add_time)
    st.markdown("---")
    st.markdown(
